...
//! - an `agent_oxide::agent_kit::AgentBlueprint` impl covering the *field* half:
...
fn expand_impl(input: DeriveInput) -> syn::Result<TokenStream2> {
    ...
    let (context_inherent, context_trait) = if context_fields.is_empty() {
            ...
            .map(|(ident, mode)| {
                ...
                match mode {
                    ContextMode::Static => quote! {
                        __hook.add(agent_oxide::agent_kit::ContextBlock::static_block(
                            #key,
                            10,
                            agent_oxide::agent_kit::serde_json::to_string(&self.#ident).unwrap_or_default(),
                        ));
                    },
                    ContextMode::Dynamic => quote! {
                        {
                            let __snapshot = self.#ident.clone();
                            __hook.add(agent_oxide::agent_kit::ContextBlock::dynamic_block(
                                #key,
                                10,
                                move || {
                                    agent_oxide::agent_kit::serde_json::to_string(&__snapshot)
        ...
        let inherent = quote! {
            #[doc(hidden)]
            fn __agent_context_hook(&self) -> agent_oxide::agent_kit::ContextBlockHook {
                let mut __hook = agent_oxide::agent_kit::ContextBlockHook::new();
                #( #blocks )*
                __hook
            }
        ...
        let trait_impl = quote! {
            fn blueprint_context_hooks(&self) -> Vec<Box<dyn agent_oxide::agent_kit::engine::AgentHook>> {
    ...
    let model_expr = match &model {
        ...
        None => quote! { agent_oxide::agent_kit::DEFAULT_MODEL.to_string() },
    };
    ...
    let inherent = quote! {
        ...
        impl #name {
            ...
            ) -> Result<agent_oxide::agent_kit::engine::Agent<#client_ty>, agent_oxide::agent_kit::engine::AgentError> {
                self.into_agent_with(model, agent_oxide::agent_kit::BuildConfig::default())
            }
            ...
            pub fn into_agent_with(
                self,
                model: impl Into<String>,
                mut config: agent_oxide::agent_kit::BuildConfig,
            ) -> Result<agent_oxide::agent_kit::engine::Agent<#client_ty>, agent_oxide::agent_kit::engine::AgentError> {
                ...
                let mut __hooks = agent_oxide::agent_kit::AgentBlueprint::blueprint_context_hooks(&self);
                __hooks.extend(config.extra_hooks.drain(..));
                let __config = agent_oxide::agent_kit::BuildConfig { extra_hooks: __hooks, ..config };
                let __system_prompt = self.agent_system_prompt();
                agent_oxide::agent_kit::AgentAssembler::new(self.#client_field, model.into())
                    .system_prompt(__system_prompt)
                    .tools(move |__reg: &mut agent_oxide::agent_kit::tools::ToolRegistry| {
    ...
    let blueprint = quote! {
        #[automatically_derived]
        impl agent_oxide::agent_kit::AgentBlueprint for #name {
