...
//!    is replaced with an LLM call via `agent_oxide::agent_kit::run_generation`, and the
//!    return type is wrapped: `-> T` becomes
//!    `-> Result<T, agent_oxide::agent_kit::GenerationError>`.
//! 3. An `agent_oxide::agent_kit::AgentBlueprint` impl covering the *method* half
        ...
        quote! {
            ...
            impl #struct_ty {
                ...
                fn blueprint_register_method_tools(
                    &self,
                    __registry: &mut agent_oxide::agent_kit::tools::ToolRegistry,
                ) {
                    #( #tool_registrations )*
                }
...
fn transform_generation_method(mut method: ImplItemFn) -> syn::Result<TokenStream2> {
        ...
        ReturnType::Default => {
            return Err(syn::Error::new_spanned(
                ...
                "generation methods must declare a return type \
                 (the macro wraps it in `Result<T, agent_oxide::agent_kit::GenerationError>`)",
            ));
        }
    };
    if is_result_type(&method.sig.output) {
        return Err(syn::Error::new_spanned(
            ...
            "declare the plain value type — `#[agent_impl]` wraps it in \
             `Result<T, agent_oxide::agent_kit::GenerationError>` automatically",
        ));
    }
    ...
    let (strategy_expr, use_tools) = match &strategy {
        Strategy::Predict { max_retries } => (
            quote! {
                agent_oxide::agent_kit::Strategy::Predict { max_retries: #max_retries }
            },
            false,
        ),
        Strategy::CodeAct {
            ...
            quote! {
                agent_oxide::agent_kit::Strategy::CodeAct {
    ...
    let registry_stmts = if use_tools {
        quote! {
            let mut __registry = agent_oxide::agent_kit::tools::ToolRegistry::new();
            agent_oxide::agent_kit::AgentBlueprint::blueprint_register_tools(self, &mut __registry);
        }
    } else {
        TokenStream2::new()
    };
    ...
    let body: Block = syn::parse_quote! {
        {
            ...
            #registry_stmts
            agent_oxide::agent_kit::run_generation::<_, #ret_ty>(
                self.agent_client(),
                &self.agent_model(),
                &__system,
                &__prompt,
    ...
    method.sig.output = syn::parse_quote! {
        -> Result<#ret_ty, agent_oxide::agent_kit::GenerationError>
    };
...
fn make_tool_adapter(
    ...
    let args_struct = if args.is_empty() {
        quote! {
            ...
            #[derive(agent_oxide::agent_kit::serde::Deserialize, agent_oxide::agent_kit::schemars::JsonSchema)]
    ...
    let call = if is_result_type(&method.sig.output) {
        quote! {
            let __result = match self.agent.#name(#(args.#arg_idents),*) {
                Ok(__v) => __v,
                Err(__e) => {
                    return Err(agent_oxide::agent_kit::tools::ToolError::Execution(__e.to_string()));
                }
            };
        }
    } else {
    ...
    let adapter = quote! {
        ...
        impl agent_oxide::agent_kit::tools::Tool for #tool_struct_name {
            fn name(&self) -> &str {
                #tool_name
            }
            ...
            fn parameter_schema(&self) -> agent_oxide::agent_kit::serde_json::Value {
                static __SCHEMA: ::std::sync::OnceLock<agent_oxide::agent_kit::serde_json::Value> =
                    ::std::sync::OnceLock::new();
                __SCHEMA
                    .get_or_init(|| agent_oxide::agent_kit::tools::generate_schema::<#args_struct_name>())
                    .clone()
            }
            ...
            fn execute_stream(
                &self,
                raw_args: &str,
            ) -> Result<agent_oxide::agent_kit::tools::ProgressStream, agent_oxide::agent_kit::tools::ToolError> {
                let args: #args_struct_name = agent_oxide::agent_kit::serde_json::from_str(raw_args)
                    .map_err(|e| {
                        agent_oxide::agent_kit::tools::ToolError::InvalidArgs(format!("invalid args: {e}"))
                    })?;
                #call
                let __json = agent_oxide::agent_kit::serde_json::to_string(&__result).map_err(|e| {
                    agent_oxide::agent_kit::tools::ToolError::Execution(format!("serialize result: {e}"))
                })?;
                Ok(agent_oxide::agent_kit::tools::ProgressStream::done(__json))
            }
        }
    };
