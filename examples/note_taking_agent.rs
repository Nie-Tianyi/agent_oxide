...
fn validate_context(agent: &NoteTakingAgent) {
    ...
    let hooks = agent_oxide::agent_kit::AgentBlueprint::blueprint_context_hooks(agent);
    assert_eq!(hooks.len(), 1, "one hook for the notes field");
    let mem = std::sync::Arc::new(std::sync::RwLock::new(
        agent_oxide::agent_kit::memory::Memory::new(),
    ));
...
impl NoteTakingAgent {
    ...
    async fn answer_count(
        &self,
        question: String,
    ) -> Result<NoteCount, agent_oxide::agent_kit::GenerationError> {
        ...
        agent_oxide::agent_kit::run_generation::<_, NoteCount>(
            self.agent_client(),
            &self.agent_model(),
            &system,
            &prompt,
