...
impl<C: LLMClient> AgentHook for MacroCompactHook<C> {
    ...
    fn on_llm_start(&self, _session_id: &str, memory: &SharedMemory) {
        ...
        // `crate::engine::block_on` — a bare `Handle::block_on` would panic here
...
fn truncate(s: &str) -> String {
    ...
    let cut = crate::util::floor_char_boundary(s, PLACEHOLDER_ARG_TRUNCATE);
    format!("{}…", &s[..cut])
}
...
mod tests {
    use super::*;
    use crate::provider::{ToolCall, ToolCallFunction, ToolCallKind};
