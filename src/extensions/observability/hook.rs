...
use crate::engine::{AgentError, AgentHook};
use crate::memory::SharedMemory;
use crate::provider::{Message, ProviderError, ToolCall};

use super::{TraceEvent, TraceStore};
...
impl AgentHook for ObservabilityHook {
    ...
    fn on_run_start(&self, session_id: &str, user_input: &str, _memory: &SharedMemory) {
        ...
        let truncated = if user_input.len() > 200 {
            let boundary = crate::util::floor_char_boundary(user_input, 200);
    ...
    fn on_run_finish(
        &self,
        _session_id: &str,
        outcome: &crate::engine::RunOutcome,
        ...
        let outcome_str = match outcome {
            crate::engine::RunOutcome::Success { answer: _ } => "success".to_string(),
            crate::engine::RunOutcome::Error { error } => format!("error: {error}"),
