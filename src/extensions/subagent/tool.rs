...
use crate::engine::{Agent, EngineContext};
use crate::memory::{Memory, SharedMemory};
use crate::observability::{ObservabilityHook, TraceEvent, TraceStore};
use crate::provider::{LLMClient, Message, Role};
use crate::tools::{Progress, ProgressStream, Tool, ToolError, ToolRegistry, generate_schema};
...
impl<C: LLMClient + Clone + 'static> Tool for SubagentTool<C> {
    ...
    fn parameter_schema(&self) -> serde_json::Value {
        ...
        SCHEMA.get_or_init(generate_schema::<TaskArgs>).clone()
    }

    fn execute_stream(&self, raw_args: &str) -> Result<ProgressStream, ToolError> {
        let args: TaskArgs = serde_json::from_str(raw_args)
            .map_err(|e| ToolError::InvalidArgs(format!("invalid args: {e}")))?;
        SubagentTool::execute_stream(self, args)
    }
}
...
// ── execute_stream (called by the manual Tool impl above) ─────────────────────
            ...
            event = sub_rx.recv() => {
                match event {
                    Some(evt) => {
                        ...
                        if matches!(&evt, crate::engine::AgentEvent::ToolCall { .. }) {
                            tool_call_count += 1;
                        }
                        if matches!(&evt, crate::engine::AgentEvent::RunCompleted { .. }
                            | crate::engine::AgentEvent::RunFailed { .. })
                        {
...
/// Map a sub-agent [`crate::engine::AgentEvent`] to a [`Progress`] event.
///
...
/// `Progress::Done`.
fn forward_event_to_progress(
    event: crate::engine::AgentEvent,
    ...
    match event {
        ...
        AgentEvent::ToolCall {
            ...
            let args_summary = if arguments.len() > TRUNCATE_ARGS_CHARS {
                let boundary = crate::util::floor_char_boundary(&arguments, TRUNCATE_ARGS_CHARS);
...
fn summarize_output(output: &str) -> String {
    ...
    if trimmed.len() > OUTPUT_SUMMARY_CHARS {
        let boundary = crate::util::floor_char_boundary(trimmed, OUTPUT_SUMMARY_CHARS);
