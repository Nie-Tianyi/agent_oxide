...
pub fn save_conversation(
    ...
    let cf = ConversationFile {
        ...
        saved_at: crate::util::iso8601_now(),
...
pub fn sanitize_filename(name: &str) -> String {
    ...
    let end = crate::util::floor_char_boundary(name, MAX_THREAD_NAME_CHARS.min(name.len()));
    ...
    if guarded.is_empty() {
        format!(
            "conversation-{}",
            crate::util::iso8601_now().replace([':', 'T'], "-")
        )
    } else {
        guarded
    }
...
mod tests {
    ...
    fn today_date() -> String {
        crate::util::iso8601_now()[..10].to_string()
    }
